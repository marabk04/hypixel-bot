...
class division_command(commands.Cog):
    ...
    async def division_command(self, interaction: discord.Interaction, x:float, y:float):
        try:
            result = x/y
            if result % 1 == 0:
...
async def setup(bot):
    await bot.add_cog(division_command(bot))