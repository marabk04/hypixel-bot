...
def display_current_election_embed(candidate_data, election_year, total_votes):
...
def get_election_ended_data():
  candidate_data = []
  mayor_data = []
  mayor_perks = []
  total_votes = sum(candidate['votes'] for candidate in data["mayor"]["election"]["candidates"])
  election_year = data["mayor"]["election"]["year"]


  mayor_name = data['mayor']['name']
  mayor_key = data['mayor']['key']
  for perk in data['mayor']['perks']:
    mayor_perk_name = perk['name']
    mayor_perk_description = perk['description']
    mayor_perks.append((mayor_perk_name, mayor_perk_description))

    mayor_info = {'name': mayor_name, 'perks': mayor_perks, 'key': mayor_key}
    mayor_data.append(mayor_info)
  ...
  return mayor_data, candidate_data, election_year, total_votes
  


def display_election_ended_embed(mayor_data, candidate_data, election_year, total_votes):
    max_votes_candidate = max(candidate_data, key=lambda candidate: candidate['votes'])
    max_votes_percentage = (max_votes_candidate['votes'] / total_votes) * 100
    mayor = mayor_data[0]
    ...
    embed = discord.Embed(title = f"<:tada:1197289598670356562> {mayor['name']} has won the election <:tada:1197289598670356562>", description= f"{mayor['name']} won the election with **``{max_votes_percentage:.2f}%``** of votes", colour=0x00b0f4)
    embed.set_thumbnail(url=output_skin(mayor["name"]))
    embed.set_footer(text=f"Election Year {election_year} • Last Updated at {datetime.now().strftime('%I:%M %p')} ")
    


    for mayor in mayor_data:
        for perk_name, perk_description in mayor['perks']:
            embed.add_field(name=f"• {remove_minecraft_formatting(perk_name)}", value=remove_minecraft_formatting(perk_description), inline=False)
...
class auto_update_election_command(commands.GroupCog, name="election"):
    ...
    async def send_or_update_election(self):
        election_year = data["mayor"]["election"]["year"]
        if election_year is False:
            candidate_data, election_year, total_votes = get_data()
            embed = display_current_election_embed(candidate_data, election_year, total_votes)
        else:
            mayor_data, candidate_data, election_year, total_votes = get_election_ended_data()
            embed = display_election_ended_embed(mayor_data, candidate_data, election_year, total_votes)
    ...
    async def election_commandd(self, interaction: discord.Interaction):
        try:
            candidate_data, election_year, total_votes = get_data()
            ...
            embed = discord.Embed(description="There is no current election", color=0x00b0f4, timestamp=datetime.now())
